...
export default class Validator {
		...
		return data.filter((elm) => {
			return (
				objs.filter((obj) => {
					for (let prop in obj) {
						return (
							elm.hasOwnProperty(prop) && obj[prop] === elm[prop]
						);
					}
				}).length > 0
			);
	...
	setFileName(filename) {
		...
		if (
			!fs.existsSync(
				path.join(__dirname, '../../', folderPaths.raw, this.filename)
			)
		) {
			throw new ServerError(
				400,
				`Filename ${this.filename} cannot be found on the server.`
			);
		}
	}
	...
	setElectionDate(electionDate) {
		try {
			...
			throw new ServerError(
				400,
				'Missing election date or wrong format. Expecting YYYY-MM-DD.',
				e
			);
		}
	}
	...
	setRow(row) {
		for (let i = 0; i < this.schema.length; i++) {
			...
			if (schemaObj === undefined || schemaObj.value === '') {
				throw new ServerError(
					400,
					`Missing Field for ${this.schema[i].name}.`
				);
			}
		}
		this.row = row;
